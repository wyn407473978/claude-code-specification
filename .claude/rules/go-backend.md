...
- 配置管理：`Viper`
- ORM：`Gorm`
- 数据库：默认支持 `MySQL`，如需求指定其他数据库，需保持 Repository 层接口稳定
...
整个后端项目必须遵循 TDD（Test-Driven Development，测试驱动开发）模式。

核心原则：

```text
核心业务逻辑必须遵循 TDD：没有失败测试，不允许编写对应生产代码。
```
...
3. 核心业务逻辑必须先为目标行为编写失败测试，并运行测试确认失败原因正确
...
14. 如果当前环境具备 Git 仓库和提交权限，每次完成一个独立修改后必须提交，提交信息必须使用中文
15. 如果当前环境无法提交，必须在交付说明中明确原因，并给出推荐中文提交信息
...
- 禁止在具备 Git 仓库和提交权限时，修改项目代码后不进行 Git 提交
- 禁止使用英文提交信息作为正式提交 message
- 禁止把多个不相关修改混在同一次提交中
...
17. 运行 `gofmt`
18. 运行 `go test ./...`
19. 如果当前环境具备提交条件，使用 Git 提交本次代码修改，提交信息必须为中文
20. 如果无法提交，在交付说明中写明原因和推荐提交信息

## 28. 本地开发热更新规范

后端项目本地开发必须使用 `Air` 进行热更新，提高开发和 Agent 调试效率。

要求：

- 项目根目录必须提供 `.air.toml`
- `Air` 只用于本地开发环境，禁止作为生产启动方式
- `.air.toml` 必须监听 Go 源码、配置文件和必要模板文件
- `.air.toml` 必须排除 `.git`、`tmp`、`vendor`、`docs`、`migrations` 等不需要触发重启的目录
- 热更新构建产物必须输出到临时目录，例如 `tmp`
- README 必须提供 Air 安装方式和启动命令
...
Agent 新建后端项目时，必须同时创建 `.air.toml` 并在中文 `README.md` 中说明热更新启动方式。
...
如果当前环境具备 Git 仓库和提交权限，Agent 每次完成一个独立修改后必须提交。

要求：

- 每次完成一个明确功能、bug 修复、重构、文档或配置调整后，必须执行一次 Git 提交
- 提交前必须先完成格式化、测试和 README 同步检查
- 提交信息必须使用中文
- 提交信息必须说明本次修改的业务目的或技术目的
- 一次提交只包含一个明确主题，禁止混入多个不相关修改
- 如果本次修改只更新规范、文档或配置，提交信息也必须明确说明
- 如果当前环境无法提交，必须在交付说明中明确说明原因，并给出推荐中文提交信息
...
- 项目可以正常启动
- 核心业务新增或变更行为均遵循 TDD，并有对应测试记录
- 核心业务新增测试已确认经历过失败阶段，然后通过
- 代码已按职责拆分
- 命名清晰统一
- Gin 路由注册清晰
- Viper 配置集中管理
...
- 本地开发已配置 Air 热更新，或本次变更不涉及项目启动配置
- 已执行格式化和测试
- 如果当前环境具备 Git 仓库和提交权限，已使用中文提交信息提交本次独立修改
- 如果当前环境无法提交，已在交付说明中明确原因并给出推荐中文提交信息
